...
- Sibling slice plans are the current micro-contract (types, call stacks, assertion, must-not, verification).
...
- `start --model` records attribution.
  ...
  The `edit-code` packet tells the coding subagent to load and follow volod-style before the first edit; that does not replace the later review pass.
...
(types, call stacks, assertion, must-not, verification).
